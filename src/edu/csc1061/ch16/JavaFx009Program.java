...
public class JavaFx009Program extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        scrollBarHorizontal.valueProperty()
                .addListener(observable -> text.setX(scrollBarHorizontal.getValue()
                        * paneForText.getWidth() / scrollBarHorizontal.getMax()));

        // Listener for vertical scroll bar value change
        scrollBarVertical.valueProperty()
                .addListener(observable -> text.setY(scrollBarVertical.getValue()
                        * paneForText.getHeight() / scrollBarVertical.getMax()));
