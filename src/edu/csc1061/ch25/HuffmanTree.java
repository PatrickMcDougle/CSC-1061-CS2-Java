...
public class HuffmanTree implements Comparable<HuffmanTree> {
  ...
  public HuffmanTree(HuffmanTree tree1, HuffmanTree tree2) {
    this(
        tree1.getRoot().getElement().getWeightValue()
            + tree2.getRoot().getElement().getWeightValue(),
        (char) 0);
  ...
  public int compareTo(HuffmanTree that) {
    if (this.root.getElement().getWeightValue()
        < that.root.getElement().getWeightValue()) { // Purposely reverse the
      // order
      return 1;
    }
